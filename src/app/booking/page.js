...
export default function BookingPage() {
  ...
  const FORM_ACTION =
    "https://docs.google.com/forms/d/e/1FAIpQLSdhwXOpE2eFYh26SeedJgWlx1dObtVkB81jOJi3Y6l_Cf1p-w/formResponse";

  // IDs ikut link yang kau bagi
  const ENTRY = {
    selectedCar: "entry.1242793116",
    name: "entry.1589861015",
    email: "entry.1298682969",
    phone: "entry.38197127",
    pickupLocation: "entry.1045954874",
    pickupDate: "entry.1063230868",
    pickupTime: "entry.733215374",
    dropLocation: "entry.485806253",
    dropDate: "entry.1853447200",
    dropTime: "entry.471796680",
    notes: "entry.1114137327",
  };
  ...
  const carOptions = [
    ...
    { name: "BMW", price: "RM400.00 /Per Trip", image: "/car-img/bmw.png", category: "Luxury Sedan", capacity: "Up to 3 passengers", ideal: "VIPs and special occasions" },
  ];
      ...
      <section className="bg-white py-16 px-6 sm:px-12">
        ...
        <section className="bg-white px-6 sm:px-12 pt-10 max-w-5xl mx-auto text-center">
          ...
          <p className="text-red-600 text-sm mt-4 font-semibold">
            *All prices listed are from Kuala Lumpur International Airport (KLIA/KLIA2/Subang) to Klang Valley (Kuala Lumpur) only.
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 max-w-6xl mx-auto">
          {carOptions.map((car, i) => (
            <div key={i} className="border rounded-xl shadow-lg overflow-hidden bg-white hover:shadow-2xl transition">
      ...
      <section id="urgent-booking" className="bg-[#1e1e1e] py-16 px-6 sm:px-12">
        <div className="max-w-4xl mx-auto bg-[#2b2b2b] border border-gray-700 rounded-2xl shadow-2xl p-10 space-y-8">
          <div className="text-center">
            ...
            {/* Remark duplicate near form */}
            <p className="text-red-400 text-xs mt-3">
              *Prices are valid for Airport ↔ Klang Valley (Kuala Lumpur) only.
            </p>
          </div>
          ...
          <form
            onSubmit={async (e) => {
              ...
              const pickupDate = form.pickupDate.value;   // yyyy-mm-dd
              const pickupTime = form.pickupTime.value;   // HH:mm
              const dropLocation = form.dropLocation.value;
              const dropDate = form.dropDate.value;       // yyyy-mm-dd
              const dropTime = form.dropTime.value;       // HH:mm
              const notes = form.notes.value;

              const formData = new FormData();
              formData.append(ENTRY.selectedCar, selectedCar);
              formData.append(ENTRY.name, name);
              formData.append(ENTRY.email, email);
              formData.append(ENTRY.phone, phone);
              formData.append(ENTRY.pickupLocation, pickupLocation);
              formData.append(ENTRY.pickupDate, pickupDate);
              formData.append(ENTRY.pickupTime, pickupTime);
              formData.append(ENTRY.dropLocation, dropLocation);
              formData.append(ENTRY.dropDate, dropDate);
              formData.append(ENTRY.dropTime, dropTime);
              formData.append(ENTRY.notes, notes);
